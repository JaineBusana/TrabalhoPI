...
namespace Trabalho_PI.Model
{
    public class CategoriaResiduoModel : Database, ICrud 
    {
        public void Create()
        {
            UserEntity categoriaResiduo = new UserEntity();
        ...
        {
            UserEntity categoriaResiduo = Popular(GetById());
        ...
        public UserEntity GetById(int id = 0)
        {
            if (id == 0)
            {
                id = GetIndex();
        ...
        private IEnumerable<UserEntity> ListCategoriaResiduoEntity()
        {
            string sql = @"SELECT * 
                           FROM CATEGORIA_RESIDUO 
                           JOIN RESIDUO
                           ON  RESIDUO.ID = RESIDUO_ID ";
            return this.GetConnection().Query<UserEntity, ResiduoEntity, UserEntity>(
                sql,
        ...
        private UserEntity Popular(UserEntity categoriaResiduo)
        {
        ...
        private int ChangeResiduo(UserEntity categoriaResiduo)
        {
